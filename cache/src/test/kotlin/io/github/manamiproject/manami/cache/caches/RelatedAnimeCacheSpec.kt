...
object RelatedAnimeCacheSpec : Spek({
    ...
    given("an empty cache") {
        val remoteRetrievalMock = mock<RemoteFetcher> {
            ...
            } doReturn setOf(InfoLink("${MAL.url}2994"))
        }
    ...
    given("a cache populated with two entries") {
        ...
        val deathNoteRelatedAnime = mutableSetOf(InfoLink("${MAL.url}2994"))
        cache.populate(deathNoteInfoLink, deathNoteRelatedAnime)

        val madeInAbyssInfoLink = InfoLink("${MAL.url}34599")
        cache.populate(madeInAbyssInfoLink, mutableSetOf(InfoLink("${MAL.url}36862")))
